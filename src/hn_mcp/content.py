...
# Blocked private IP ranges for security (like GeorgeNance's implementation)
...
def _extract_article_content(html: str, url: str) -> str:
    """
    ...
    2. Look for common content class names
    ...
    if not content:
        for selector in ["main", ".content", ".post-content", ".article-content", ".entry-content", "#content"]:
...
async def fetch_article_content(url: str) -> dict[str, str]:
    ...
    try:
        async with httpx.AsyncClient(
            ...
            headers={
                "User-Agent": "Mozilla/5.0 (compatible; hn-mcp/1.0; +https://github.com/machinemates-ai/hn-mcp)",
            ...
            if content_length and int(content_length) > MAX_CONTENT_SIZE:
                raise ContentExtractionError(f"Content too large: {content_length} bytes")
            ...
            content = _extract_article_content(html, url)
        ...
        raise ContentExtractionError(f"HTTP {e.response.status_code}: {url}")
    except httpx.TimeoutException:
        raise ContentExtractionError(f"Timeout fetching: {url}")
    except httpx.RequestError as e:
        raise ContentExtractionError(f"Request error: {e}")
